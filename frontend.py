...
def bootinput(password,adapter,adapter2,nfs,mac_c):
    ...
    if inp==1:  # $1--adapter $2--password $3--mac_c--mac_file
        subprocess.run([f'bash','xcatstateless_files/xCAT_stateless_Installation.sh',f'{adapter}',f'{password}',f'{mac_c}',f'{nfs}'])
        ready_nodes="no"
        while ready_nodes != "yes":
            ready_nodes=input("\n   START THE NODES ""YES OR NO ---> ")
        # $1-- mac_c --mac_file $2-- hostonly network adapter of compute nodes
        subprocess.run([f'bash','xcatstateless_files/xcat_internet_nating.sh',f'{mac_c}',f'{adapter2}'])

    elif inp==2:
        subprocess.run([f'bash','xcatstatefull_files/xCAT_statefull_Installation.sh',f'{adapter}',f'{password}',f'{mac_c}',f'{nfs}'])
        ready_nodes="no"
        while ready_nodes != "yes":
            
            ready_nodes=input("\n   START THE NODES ""YES OR NO ---> ")
        #script 2 # $1--mac_c--mac_file #$2-- hostonly network adpater of compute nodes
        subprocess.run([f'bash','xcatstatefull_files/xcat_statefull_nagios_installation.sh',f'{mac_c}',f'{adapter2}'])
        subprocess.run([f'bash','xcatstatefull_files/xcat_statefull_slurm_installation.sh',f'{mac_c}',f'{adapter2}'])
    else:
